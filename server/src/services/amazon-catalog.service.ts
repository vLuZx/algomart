...
import type { CatalogSearchResponse, CatalogItem } from '../types/amazon.types.js';
...
class AmazonCatalogService {
  ...
  async searchByIdentifier(
    ...
    try {
      const response = await amazonClient.get<CatalogSearchResponse>(
        `/catalog/${this.API_VERSION}/items`,
        {
  ...
  async searchByBarcode(barcode: string, barcodeType: string): Promise<CatalogItem | null> {
    const identifierType = barcodeTypeToIdentifierType(barcodeType as any);
  ...
  async getByAsin(asin: string): Promise<CatalogItem | null> {
    ...
    try {
      const response = await amazonClient.get<CatalogItem>(
        `/catalog/${this.API_VERSION}/items/${asin}`,
        {
          marketplaceIds: marketplaceId,
